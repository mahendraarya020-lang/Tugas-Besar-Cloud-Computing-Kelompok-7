...
<body class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md space-y-6">
        ...
        <div class="bg-white rounded-2xl p-8 border border-brand-outline card-shadow card-shadow-hover transition-all duration-300">
            <h2 class="text-xl font-bold text-[#191c1e] mb-5 text-center">Create Account</h2>
            ...
            <form action="/register" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label for="name" class="block text-xs font-semibold text-[#191c1e] uppercase tracking-wider mb-1.5">Full Name</label>
                ...
                <div>
                    <label for="role" class="block text-xs font-semibold text-[#191c1e] uppercase tracking-wider mb-1.5">Select Role</label>
                    <select id="role" name="role" required 
                            class="w-full px-3.5 py-2.5 rounded-lg bg-white border border-[#cbd5e1] text-[#191c1e] focus:outline-none focus:ring-1 focus:ring-[#24389c] focus:border-[#24389c] transition-all text-sm">
                        <option value="User" {{ old('role') == 'User' ? 'selected' : '' }}>User (Freelancer)</option>
                        <option value="Admin" {{ old('role') == 'Admin' ? 'selected' : '' }}>Admin (System Overseer)</option>
                    </select>
                </div>
            ...
            <div class="mt-5 text-center text-xs text-brand-secondary">
                ...
                <a href="/login" class="text-[#24389c] hover:text-[#1a2c80] font-semibold underline underline-offset-2 transition-colors">
                    Sign In
                </a>
            </div>
        </div>
